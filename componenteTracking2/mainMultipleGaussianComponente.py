...
#sys.path.insert(0, '/home/marc/Escriptori/kerasTutorial/customGenerator')
...
PATH_SAVE = '/home/marc/Dropbox/tfmDeepLearning/semana8/componente/data'
...
weights_path = '/home/marc/Escriptori/siameseDATASET/FirstApproach/convModel/model3/main1weights-improvement-24.hdf5'
modelSiamese.load_weights(weights_path)

adam = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.00005)


modelSiamese.compile(loss='binary_crossentropy',
            optimizer=adam,
...
#for x in range(1,31):
    ...
    if x == listOfDetections[indexDetections]:
        ...
        #
        #numLosItera = np.shape(listLostROI)
        #for ss in range(0,numLosItera[0]):
        #    cv2.imwrite('lost'+str(ss)+'.jpg',listLostROI[ss])
        ...
        print(detecction)
        print('lost',numLostRois,'det',numDeteccionesSin)
        
        for xs in range(0,numDeteccionesSin[0]):
            print('loooooop',xs,numDeteccionesSin[0],numLostRois[0])
            ...
            if  numLostRois[0] == 0:
                ...
                print('sasa',randomIdentities[xs,:])

                randomIdentities = np.delete(randomIdentities, xs, 0)
                colors = np.delete(colors, xs, 0)
                #print('nooooooooooooooooooooo matcheddddddddddddd')
                continue

            for iterLost in range(0,numLostRois[0]):       

                #print(xs,numDeteccionesSin[0],iterLost,numLostRois[0])

                imagePos_1 = listLostROI[iterLost]
                #imagePos_1 = cv2.cvtColor(imagePos_1, cv2.COLOR_BGR2RGB)
                print('res',np.shape(imagePos_1))
                ...
                #imagePos_2 = cv2.cvtColor(imagePos_2, cv2.COLOR_BGR2RGB)
                print('res',np.shape(imagePos_2))
            ...
            #print('selec',matrixResults[0,idxMajorSimiliratiry],idxMajorSimiliratiry)

            # the most similiraity and over a threshold
            if matrixResults[0,idxMajorSimiliratiry]>0.7:
                ...
                cv2.imwrite('lost'+str(iterLost)+'-'+str(x)+'.jpg',listLostROI[idxMajorSimiliratiry])
                cv2.imwrite('det'+str(iterLost)+'-'+str(x)+'.jpg',imagePos_2)
            ...
            print('lost',numLostRois[0],'det',xs,numDeteccionesSin)
    ...
    for iPerson in range(0,numPersonas):
        ...
        print(n21,dx,dy,vxPorcTemporal,vyPorcTemporal,tracas)
        ...
        if (vxPorcTemporal >= limitBOTH and vyPorcTemporal >=limitBOTH) or tracas == 1 or vyPorcTemporal > limitTemporal or vxPorcTemporal > limitTemporal:
            ...
            print('mateu',x,iPerson,vxPorcTemporal,tracas,vyPorcTemporal)
            print('eraseddd',np.shape(roiFrame1)[0],np.shape(roiFrame1)[1],roiID[iPerson][0])

            if np.shape(roiFrame1)[0] != 0 | np.shape(roiFrame1)[1] != 0:
            ...
            #print('ssssssProblem',iPerson,numPersonas)
            #print(roiID[iPerson])
            #print(str(roiID[iPerson][0]))
            #print(int(roi[iPerson,0]))
            #print(int(roi[iPerson,1]))
            cv2.putText(framePintar,str(roiID[iPerson][0]),(int(roi[iPerson,0]),int(roi[iPerson,1])),cv2.FONT_HERSHEY_SIMPLEX, 1,(255,0,0),1,cv2.LINE_AA) 
...
#np.savetxt('/home/marc/Dropbox/tfmDeepLearning/semana7/componenteSSDtf/MOT16-13.txt', results, delimiter=',',fmt="%d") 
